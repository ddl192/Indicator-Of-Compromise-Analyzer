...
def save_alerts(alerts, outfile):  # Function to save alerts to a CSV file
    outdir = os.path.dirname(outfile)  # Get the output file directory
    if outdir and not os.path.exists(outdir):  # If the directory is specified and does not exist
        print(f"Output directory '{outdir}' does not exist.")  # Print error message
        return
    try:
        with open(outfile, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=['file', 'ioc_type', 'pattern', 'line'])  # Create a CSV DictWriter
            writer.writeheader()  # Write the header row
            for a in alerts:  # For each alert
                writer.writerow(a)  # Write the alert to the CSV
    except PermissionError:
